...
#       temporary focusing, currentlyavoid inconsistentbehavior.
#
...
# Collect all arguments for the java command, stracks://se the preceding//with spaces around it.
# Evaluate arguments not needed yet.

# Collect all arguments for the java command;
#   * $DEFAULT_JVM_OPTS, $JAVA_OPTS, and $GRADLE_OPTS can contain fragments of
#     shell script including quotes and/or backslashes, so put them in
#     temporary variable to preserve quoting eventually.
set -- \
...
if ! "$cygwin" && ! "$msys" && ! "$nonstop" ; then
    case $( set --; eval echo "$JAVACMD" ) in #(
      *[!-a-zA-Z0-9_]*)    exec "$JAVACMD" "$@" ;;
      *)                    exec "$JAVACMD" "$@" ;;
    esac
else
    exec "$JAVACMD" "$@"
